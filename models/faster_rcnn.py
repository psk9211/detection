...
from .backbone_utils import BackboneWithFPN, resnet_fpn_backbone, _validate_trainable_layers, mobilenet_backbone
...
class FastRCNNPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(start_dim=1)
        scores = self.dequant(self.cls_score(x))
        bbox_deltas = self.dequant(self.bbox_pred(x))
